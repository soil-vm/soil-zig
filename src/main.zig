...
pub fn main() !void {
    ...
    var rest: ArrayList([]const u8) = .{};
    while (args.next()) |arg| try rest.append(alloc, arg);
...
fn init_process_args(alloc: Alloc) !void {
    process_args = ArrayList([]const u8){};
    var args_iter = std.process.args();
    _ = args_iter.skip(); // Skip the first arg, which is just the soil invocation.
    while (true) {
        try process_args.append(alloc, args_iter.next() orelse break);
    }
}
...
const Syscalls = struct {
    ...
    pub fn open_reading(vm: *Vm, filename_data: i64, filename_len: i64) callconv(.c) Syscall.OneValue {
        ...
        const filename = vm.memory[@intCast(filename_data)..][0..@intCast(filename_len)];
        syscall_log.info("filename: {s}", .{filename});
        const fd = std.posix.open(filename, .{ .ACCMODE = .RDONLY }, 0) catch {
            syscall_log.info("didn't work", .{});
            return 0;
        };
        syscall_log.info("fd: {}", .{fd});
    ...
    pub fn read(vm: *Vm, file_descriptor: i64, buffer_data: i64, buffer_len: i64) callconv(.c) Syscall.OneValue {
        ...
        const len = std.os.linux.read(
            fd,
            vm.memory[@intCast(buffer_data)..].ptr,
            @intCast(buffer_len),
        );
        return @bitCast(len);
    }
    ...
    pub fn read_input(vm: *Vm, buffer_data: i64, buffer_len: i64) callconv(.c) Syscall.OneValue {
        ...
        const len = std.posix.read(
            std.posix.STDIN_FILENO,
            vm.memory[@intCast(buffer_data)..][0..@intCast(buffer_len)],
        ) catch {
